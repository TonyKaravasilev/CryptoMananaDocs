...
in some cases allow the sequence to be repeatable by passing the same initial configuration value. These types of
...
algorithms for PRNG are divided into three main branches of the software random data generator realizations:
...
- Quasi-random number generator (QRNG) - is a sub-random generator that produce a low-discrepancy sequence and uniformly
  ...
  domain more evenly and quite faster than other algorithms. They tend to be very useful in financial mathematics and
...
each type and can be seen in Figure 1.
